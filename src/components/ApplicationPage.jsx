...
function ApplicationPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch("https://formspree.io/f/xgoljnar", {
        method: "POST",
        ...
        <div className="bg-white p-8 md:p-12 rounded-3xl shadow-xl shadow-slate-200/60 border border-slate-100">
          <form className="space-y-6" onSubmit={handleSubmit}>
            ...
            <div className="relative mt-4">
              <button type="submit" disabled={status.type === "loading"} className="w-full bg-green-600 hover:bg-green-700 text-white font-black py-4 rounded-xl transition-all shadow-lg shadow-green-200 active:scale-[0.98] disabled:opacity-50 flex items-center justify-center gap-2">
                {status.type === "loading" ? <div className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin" /> : <><FiSend /> Submit Application</>}
              </button>

              {status.message && status.type !== "loading" && (
                <div className={`absolute inset-0 flex flex-col items-center justify-center rounded-xl overflow-hidden shadow-inner ${status.type === "error" ? "bg-red-600 text-white animate-shake" : "bg-green-700 text-white"}`}>
                  <div className="flex items-center gap-3 font-bold px-8 text-center leading-tight">
                    {status.type === "success" ? <FiCheckCircle size={24} /> : <FiAlertCircle size={24} />}
                    {status.message}
                  </div>
                  <button type="button" onClick={() => setStatus({type: "", message: ""})} className="absolute right-3 top-1/2 -translate-y-1/2 p-2 hover:bg-white/10 rounded-full transition-colors"><FiX size={20} /></button>
